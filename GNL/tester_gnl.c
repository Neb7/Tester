...
/*   Updated: 2025/12/03 15:29:17 by benpicar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int test_single_no_nl() {
    int fd = open("single_no_nl.txt", O_RDONLY);
    if (fd < 0) return 1;
    char *line = get_next_line(fd);
...
int test_single_with_nl() {
    int fd = open("single_with_nl.txt", O_RDONLY);
    if (fd < 0) return 1;
    char *line = get_next_line(fd);
...
int test_no_nl_final() {
    int fd = open("no_nl_final.txt", O_RDONLY);
    if (fd < 0) return 1;
    char *line = get_next_line(fd);
...
int test_multi_fd() {
    ...
    int fd2 = open("single_with_nl.txt", O_RDONLY);
...
int test_stress() {
    int fd = open("stress.txt", O_RDONLY);
    if (fd < 0) {
        printf("  Error: impossible d'ouvrir stress.txt\n");
        return 1;
    }
    int count = 0;
    char *line;
